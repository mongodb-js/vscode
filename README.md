...
MongoDB for VS Code registers a built-in [MongoDB MCP server](https://github.com/mongodb-js/mongodb-mcp-server) with VS Code, so AI agents such as GitHub Copilot's agent mode can query your data, inspect schemas and indexes, and manage Atlas resources through the connection you already have active in the extension.

Start the server from the command palette with **MongoDB: Start MCP Server**, then pick the `mongodb` server in your agent's tool picker. Use **MongoDB: Get MCP Server Config** to get the configuration for other MCP clients and **MongoDB: Stop MCP Server** to shut it down.

Behavior is controlled by the `mdb.mcp.*` settings — most notably `mdb.mcp.readOnly` to restrict the server to read-only tools, `mdb.mcp.disabledTools` to disable individual tools or whole categories, and `mdb.mcp.apiClientId` / `mdb.mcp.apiClientSecret` to enable the Atlas tools.

_Note: Find more details about the MongoDB GenAI Features in the [FAQ](https://www.mongodb.com/docs/generative-ai-faq/?utm_source=vscode&utm_medium=product)._
