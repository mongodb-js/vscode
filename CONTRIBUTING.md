...
1. Mark the **release as complete** in [the jira project's releases](https://jira.mongodb.org/projects/VSCODE?selectedItem=com.atlassian.jira.jira-projects-plugin%3Arelease-page&status=released-unreleased). Ensure the commits in the github release correspond to the jira release, typically this will involve renaming the `vNext` release as the recently release version, and creating a new `vNext` release.
